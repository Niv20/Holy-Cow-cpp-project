...
public:
    ...
    void drawLegend(int roomIdx, int lives, int points, char p1Inv, char p2Inv) {
        ...
        Point origin{ tl.x, tl.y }; // top-left now exactly 'L'
        ...
        auto putLine = [&](int dy, const std::string& line) { for (int i = 0; i < 16; ++i) { Point p{ origin.x + i, origin.y + dy }; if (p.x >= 0 && p.x < Screen::MAX_X && p.y >= 0 && p.y < Screen::MAX_Y) { p.draw(line[i]); } } };
